...
class Str {
    ...
    public:
        ...
        std::istream& getline(std::istream&, const char = ' ');
...
// added getline member function that adds everything in the input to the end of a string
std::istream& Str::getline(std::istream& is, const char seperator){
    while(is){
        char c;
        while (is.get(c) && c==seperator)
        ; // nothing to do, except testing the condition
        
        do this->data.push_back(c);
        while(is.get(c) && c!=seperator);
