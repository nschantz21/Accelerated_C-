...
void HCat_Pic::display(std::ostream& os, ht_sz row, bool do_pad)  const {
    // find the height difference left->height - right->height and pad that much on top and bottom
...
void Frame_Pic::display(std::ostream& os, ht_sz row, bool do_pad) const {
    // ask if requested row is in range
    if (row >= height()){
        // if out of range
        if (do_pad)
            pad(os, 0, width());
    } else { // three cases if we are
